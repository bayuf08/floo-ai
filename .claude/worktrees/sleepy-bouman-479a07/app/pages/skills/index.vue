<template>
  <div class="flex flex-col h-full overflow-hidden">
    ...
    <CreateSkillModal v-model="createOpen" />
...
function toggleSkill(skill: SkillDefinition) {
  ...
  if (!project.contextRules) {
    project.contextRules = { brandVoice: '', doGuidelines: [], dontGuidelines: [], skills: [] }
  }
  const list = project.contextRules.skills
  const existing = list.find((s) => s.id === skill.id)
  if (existing) {
    existing.active = !existing.active
  } else {
    list.push({ id: skill.id, name: skill.name, active: true })
  }
}
