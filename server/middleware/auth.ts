...
const PUBLIC_PATHS = new Set<string>([
  ...
  '/api/billing/webhook',
  // Health check + AI provider ping (so uptime monitors don't need a session)
  '/api/health',
  '/api/health/ai',
...
const PUBLIC_PATH_PREFIXES = [
  ...
  '/api/_auth/',
  // Nuxt Icon fetches icon bundles (e.g. lucide.json) at startup — these
  // are static asset endpoints owned by the library, not our API routes.
  '/api/_nuxt_icon/',
]
