...
export default defineOAuthGoogleEventHandler({
  ...
  async onSuccess(event, { user, tokens }) {
    ...
    // The on_workspace_created trigger inserts the owner-membership row
    // and the default 'free' subscription; we just need the workspaces row.
    // Skipped if the user already has any workspace (membership OR ownership).
    try {
      const [{ data: ownedRows }, { data: memberRows }] = await Promise.all([
        supabase.from('workspaces').select('id').eq('owner_id', profile.id).limit(1),
        supabase.from('workspace_members').select('workspace_id').eq('user_id', profile.id).limit(1),
      ])
      const hasAny = (ownedRows?.length ?? 0) > 0 || (memberRows?.length ?? 0) > 0
      if (!hasAny) {
        const wsName = profile.name ? `${profile.name.split(/\s+/)[0]}'s Workspace` : 'My Workspace'
        ...
        if (wsErr) {
          // Non-fatal — the user can still log in; they'll just see the
          // empty workspace dropdown and can create one manually.
          log.warn('[api/auth/callback]', 'Default workspace insert failed', {
            message: wsErr.message,
            code: (wsErr as any)?.code,
          })
        } else {
          log.info('[api/auth/callback]', 'Default workspace created for new user', {
            profileId: profile.id,
            name: wsName,
          })
        }
      ...
      log.warn('[api/auth/callback]', 'Default-workspace bootstrap threw', {
        message: (e as any)?.message,
      })
    }
