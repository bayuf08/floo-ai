...
export function rateLimit(event: H3Event, opts: RateLimitOptions) {
  ...
  if (bucket.tokens < cost) {
    ...
    // setHeader accepts `string | number`; the explicit String() cast
    // tripped the overload selection to expect a number, so we pass the
    // raw number and let Nitro stringify it.
    setHeader(event, 'Retry-After', Math.ceil(waitMs / 1000))
    throw createError({
      statusCode: 429,
