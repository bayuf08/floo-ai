    ...
    >
      ...
      <div :style="{ display: 'flex', flexDirection: 'column', gap: '10px' }">
        <div>
          ...
          />
          <p :style="helperStyle">
            Floo references your handle when writing engagement copy.
          </p>
        </div>
        <div>
          ...
          />
          <p :style="helperStyle">
            Calibrates tone for your stage —
            <strong :style="{ color: 'var(--fg-2)' }">{{ followerBandLabel }}</strong>.
          </p>
        </div>
        <div>
          ...
          />
          <div class="flex items-baseline justify-between" :style="{ marginTop: '2px' }">
            <p :style="helperStyle">
              Used to match your positioning and messaging angle.
            </p>
            <span :style="{ fontSize: '10.5px', color: 'var(--fg-3)' }">
              {{ (form.bio ?? '').length }} / 150
            </span>
          </div>
        </div>
...
/**
 * Same buckets the server uses to annotate the system prompt — keep these
 * in sync with `followerBandHint()` in `server/utils/prompt-builder.ts` so
 * the user sees the actual band Floo will read.
 */
const followerBandLabel = computed(() => {
  const n = Number(form.followers) || 0
  ...
  if (n < 1_000_000) return 'established creator — brand-safe, high production value'
